...
def add_awgn_noise(tensor, snr):
    ...
    noise = torch.normal(0.0, std_dev).cuda()
...
def add_rayleigh_noise(tensor, snr):
    ...
    # Generate noise with calculated power
    sigma = torch.sqrt(torch.abs(noise_power) / 2)+ 1e-10
...
def add_rician_noise(tensor, snr, K):
    ...
    s = torch.sqrt(torch.abs(noise_power))

    # Calculate the non-centrality parameter based on Rician K factor
    v = torch.sqrt(torch.abs(K)) * s

    X = torch.normal(torch.abs(v)+ 1e-10, torch.abs(s)+ 1e-10).cuda()
...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_z_with_awgn =  add_rician_noise(logits_z, torch.tensor(args.SNR).cuda(), K=torch.tensor(2).cuda()) # add_rayleigh_noise(logits_z, torch.tensor(args.SNR).cuda()) #
...
def plot_latent(autoencoder, data_loader, args, num_batches=100):
    for i, (x, y) in enumerate(data_loader):
        x = x.view(x.size(0), -1)
        z, x_hat, mu, logvar = autoencoder(x.to(args.device), mode='64QAM_VAE')
...
def eva_vae_generation(vib, classifier_model, dataloader_erase, args, name='test', epoch=999):
    ...
    for batch_idx, (x, y) in enumerate(dataloader_erase):
        x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
        x = x.view(x.size(0), -1)
        logits_z, x_hat, mu, logvar = vib(x, mode='64QAM_VAE')  # (B, C* h* w), (B, N, 10)
        ...
        if batch_idx == 0:
            ...
            M_for_xz = Mine1(noise_size=28 * 28, sample_size=20)
            ...
            M_for_xx = Mine1(noise_size=28 * 28, sample_size=28 * 28)
...
def unlearning_hessian(vib, args, dataloader_erase, remaining_set, reconstruction_function, classifier_model):
    ...
    if init_epoch == 0 or args.resume_training:
        ...
        for iter in range(args.num_epochs):  # self.args.local_ep
            ...
            for (images, labels), (images2, labels2) in zip(dataloader_erase, dataloader_remain2):
                ...
                vibi_f_hessian.zero_grad()
                logits_z, x_hat, mu, logvar = vibi_f_hessian(images, mode='VAE')  # (B, C* h* w), (B, N, 10)
                logits_z2, x_hat2, mu2, logvar2 = vibi_f_hessian(images2, mode='VAE')
                logits_y = classifier_model(x_hat.detach())
...
def unlearning_vae(vib, args, dataloader_erase, dataloader_remain, reconstruction_function, classifier_model, train_type):
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            logits_y2 = classifier_model(x_hat_e2.detach())
            ...
            clean_acc_temp = (logits_y2.argmax(dim=1) == y2).float().mean().item()
            acc_back_temp = (logits_y.argmax(dim=1) == y).float().mean().item()
...
print('train downstream classifer')
back_acc_list = []
...
for batch_idx, (x, y) in enumerate(test_loader):
    x, y = x.to(args.device), y.to(args.device)  # (B, C, H, W), (B, 10)
    x = x.view(x.size(0), -1)
    logits_z, logits_y, x_hat, mu, logvar = vib(x, mode='with64QAM_reconstruction')  # (B, C* h* w), (B, N, 10)
...
vibi_f_hessian = unlearning_hessian(copy.deepcopy(vib).to(args.device), args, dataloader_erase, remaining_set, reconstruction_function, classifier_model)


print('start VBU')
vbu_unl = unlearning_vae(copy.deepcopy(vib).to(args.device), args, dataloader_erase, dataloader_remain, reconstruction_function, classifier_model, train_type='VBU')
