...
def rayleigh_noise(signal, sigma=1):
    """
    Add Rayleigh noise to a signal
    """
    noise = np.random.rayleigh(sigma, size=signal.shape)  # Generate noise
    return signal + noise  # Return noisy signal



def rician_noise(signal, v, s):
    """
    Add Rician noise to a signal
    """
    noise = rice.rvs(v, s, size=signal.shape)  # Generate noise
    return signal + noise  # Return noisy signal
...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_y = self.approximator(logits_z_after_QAM)  # (B , 10)
            logits_y = logits_y.reshape((B, 10))  # (B,   10)
            return logits_z, logits_y, mu, logvar
            ...
            logits_y = self.approximator(logits_z_after_QAM)  # (B , 10)
            logits_y = logits_y.reshape((B, 10))  # (B,   10)
            x_hat = self.reconstruction(logits_z_after_QAM)
            return logits_z, logits_y, x_hat, mu, logvar
        elif mode == 'VAE':
            logits_z, mu, logvar = self.explain(x, mode='distribution')  # (B, C, H, W), (B, C* h* w)
...
def vib_train(dataset, model, loss_fn, reconstruction_function, args, epoch, mu_list, sigma_list, train_loader):
    ...
    for step, (x, y) in enumerate(dataset):
        ...
        logits_z, logits_y, x_hat, mu, logvar = model(x, mode='distribution')  # (B, C* h* w), (B, N, 10)
