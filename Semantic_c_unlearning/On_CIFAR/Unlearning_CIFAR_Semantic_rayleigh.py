...
def add_rayleigh_noise(tensor, snr):
    ...
    # Generate noise with calculated power
    sigma = torch.sqrt(torch.abs(noise_power) / 2)
    noise = torch.normal(0.0, torch.abs(sigma)+ 1e-10).cuda()
...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_z_with_awgn = add_rayleigh_noise(logits_z, torch.tensor(args.SNR).cuda())
