...
class VIB(nn.Module):
    ...
    def forward(self, x, mode='topk'):
        ...
        if mode == 'distribution':
            ...
            logits_y = self.approximator(x_hat)
...
def unlearning_vae(vib, args, dataloader_erase, dataloader_remain, reconstruction_function, classifier_model, loss_fn, train_type):
    ...
    for epoch in range(args.num_epochs):
        ...
        for (x, y), (x2, y2) in zip(dataloader_erase, dataloader_remain):
            ...
            #KL_z_r = (torch.exp(logits_z_e_log_softmax) * logits_z_e_log_softmax).sum(dim=1).mean() + math.log(logits_z_e_log_softmax.shape[1])
    ...
    print(f'VIB unlearning took {running_time} seconds')
...
args.self_sharing_rate = 10*args.unlearn_bce
...
for epoch in range(args.num_epochs):
    ...
    #
    # running_time = end_time - start_time
    # print(f'VIB Training epoch took {running_time} seconds')
